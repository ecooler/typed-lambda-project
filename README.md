...
In this programming exercise, you will implement the typechecking function, called type-of that lives in lambda-typechecker.rkt. If you look at the changes made to lambda-eval.rkt, you'll see that we first check the type to make sure an error doesn't occur--only after that do we evaluate the program (throwing away the types, in essence). You must download typed-lambda-project.zip in order to complete this project. Tests are included to make sure you've covered the various parts of type-of. 
All are required to implement typechecking on:
boolean values 
conditionals 
abstraction 
application 
